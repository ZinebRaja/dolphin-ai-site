...
import { ArrowLeft, ArrowRight, Check, Users, Package, BarChart3, TrendingUp, ShieldCheck, CheckCircle2, Sparkles } from 'lucide-react';
...
function TileBtn({ selected, onClick, children, className = '' }) {
  return (
    <button onClick={onClick} className={`roi-tile ${selected ? 'roi-tile-active' : ''} ${className}`}>
...
function EmbeddedROI() {
  ...
  return (
    <section className="roi-body container" style={{ paddingBottom: '2rem' }}>
      <div className="roi-hero-badge" style={{ marginBottom: '1.5rem' }}><Sparkles size={13} /> Free ROI Estimate</div>
      <div className="roi-layout">
        <div className="roi-questions">
          <div className="roi-question">
            <div className="roi-q-header">
              <span className="roi-q-num">1</span>
              <div><h3>What is your annual procurement spend?</h3><p>Total spend across all categories and suppliers</p></div>
            </div>
            <div className="roi-tiles-spend">
              {SPEND_OPTIONS.map((o,i) => <TileBtn key={o.label} selected={spendIdx===i} onClick={()=>setSpendIdx(i)}><strong>{o.label}</strong></TileBtn>)}
            </div>
          </div>
          <div className="roi-question">
            <div className="roi-q-header">
              <span className="roi-q-num">2</span>
              <div><h3>How many people handle spend data?</h3><p>People who prepare, clean, or analyse procurement reports</p></div>
            </div>
            <div className="roi-tiles-team">
              {TEAM_OPTIONS.map((o,i) => <TileBtn key={o.label} selected={teamIdx===i} onClick={()=>setTeamIdx(i)}><strong>{o.label}</strong><span>{o.desc}</span></TileBtn>)}
            </div>
          </div>
          <div className="roi-question">
            <div className="roi-q-header">
              <span className="roi-q-num">3</span>
              <div><h3>How mature is your current procurement process?</h3><p>Be honest — a lower score means a bigger opportunity</p></div>
            </div>
            <div className="roi-tiles-maturity">
              {MATURITY_OPTIONS.map(o => <TileBtn key={o.key} selected={maturity===o.key} onClick={()=>setMaturity(o.key)} className="roi-tile-maturity"><strong>{o.label}</strong><span>{o.desc}</span></TileBtn>)}
            </div>
          </div>
        </div>
        ...
        <div className="roi-results">
          <div className="roi-total">
            ...
            <strong className="roi-total-value">{fmtShort(total)}</strong>
            <span className="roi-total-sub">Based on {SPEND_OPTIONS[spendIdx].label} spend · {TEAM_OPTIONS[teamIdx].label} people · {mat.label}</span>
          </div>
            ...
            {results.map(r => (
              <div className="roi-result-row" key={r.label}>
                ...
                <div className="roi-result-right">
                  <strong>{fmtShort(r.value)}</strong>
          ...
          <div className="roi-bars">
            {results.map(r => (
              <div className="roi-bar-row" key={r.label}>
                ...
                <div className="roi-bar-track"><div className="roi-bar-fill" style={{ width:`${r.pct}%` }}/></div>
